...
document.getElementById('inputDataInicio').addEventListener('input', function (e) {
    let inputValue = e.target.value;
    inputValue = formatarData(inputValue);
    e.target.value = inputValue;
});

document.getElementById('inputDataTermino').addEventListener('input', function (e) {
    let inputValue = e.target.value;
    inputValue = formatarData(inputValue);
    e.target.value = inputValue;
});

document.getElementById('inputDataInicio').addEventListener('blur', function (e) {
    const inputValue = e.target.value;
    validarData(inputValue, e.target);
});

document.getElementById('inputDataTermino').addEventListener('blur', function (e) {
    const inputValue = e.target.value;
    validarData(inputValue, e.target);
});
...
document.getElementById('inputHoraInicio').addEventListener('input', function (e) {
    let inputValue = e.target.value;
    if (/\D:$/.test(inputValue)) {
        inputValue = inputValue.slice(0, -1);
    }
